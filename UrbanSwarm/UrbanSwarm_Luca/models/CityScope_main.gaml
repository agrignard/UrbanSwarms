...
global {
	...
	// GIS FILE //
	file litter_shapefile <- file(cityGISFolder+"/DPW_LitterBarrels.shp");
...
experiment CityScopeMain type: gui {	
	user_command Caculate_Mean action:find_means;
	output {	
		display CityScopeVolpe  type:opengl background:#black draw_env:true {
			species k_node aspect:base refresh:true;
			species barrel aspect:base refresh:true;
			...
			species amenityBin aspect: base;
			//species Road_Node aspect: base;
...
experiment selfOrganizedGarbageCollection type: gui {
		...
		loop x1 from:0 to: 0{
			loop x2 from:0 to: 0{
				loop x3 from:0 to: 0{
					loop x4 from:0 to: 0{
						loop x5 from:0 to: 0{
							int RN <-robotNumArray[x1];
							float EV <-evaporationArray[x2];
							float ER <-exploratoryRateArray[x3];
							int CT <-carriableTrashAmountArray[x4];
							int DN <-depositNumArray[x5];
							create simulation with:[seed::rnd(100),robotNum::RN, evaporation::EV, exploratoryRate::ER, carriableTrashAmount::CT, depositNum::DN];
							create simulation with:[seed::rnd(100),robotNum::RN, evaporation::EV, exploratoryRate::ER, carriableTrashAmount::CT, depositNum::DN];
							create simulation with:[seed::rnd(100),robotNum::RN, evaporation::EV, exploratoryRate::ER, carriableTrashAmount::CT, depositNum::DN];
							create simulation with:[seed::rnd(100),robotNum::RN, evaporation::EV, exploratoryRate::ER, carriableTrashAmount::CT, depositNum::DN];
							create simulation with:[seed::rnd(100),robotNum::RN, evaporation::EV, exploratoryRate::ER, carriableTrashAmount::CT, depositNum::DN];
	...
	output {
		...
		//monitor "trashPerTime" value: trashPerTime;
		//monitor "fullTrashBin" value: fullTrashBin;
